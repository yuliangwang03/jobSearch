...
//compared to githubAPI,GithubClient is equivalent to a front end,so we just need to use java class.

public class GitHubClient {
	private static final String URL_TEMPLATE = "https://jobs.github.com/positions.json?description=%s&lat=%s&long=%s";
	private static final String DEFAULT_KEYWORD = "developer";
	
	//Search job information based on default keywords and specific geographic location
	public List<Item> search(double lat, double lon, String keyword) {
		//corner cases
		if (keyword == null) {
			keyword = DEFAULT_KEYWORD;
		}
		//Encode the keyword to ensure that it appears in the URL after transcoding
		try {
			keyword = URLEncoder.encode(keyword, "UTF-8");
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
		//prepare HTTP requeest parameter
		String url = String.format(URL_TEMPLATE, keyword, lat, lon);
		CloseableHttpClient httpClient = HttpClients.createDefault();
		try {
			CloseableHttpResponse response = httpClient.execute(new HttpGet(url));
			//Get HTTP response body
			//If no content is returned or cannot be returned, an empty Array is returned
